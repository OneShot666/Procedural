...
namespace Terrains {
    ...
    public class Chunk : MonoBehaviour {
        ...
        private Color GetBiomeColorAt(float x, float z) {
            Color finalColor = Color.lawnGreen;

            if (ChunkBiomeGenerator.AllBiomes != null) {
                foreach (var biome in ChunkBiomeGenerator.AllBiomes) {
                    if (!biome) continue;

                    if (biome.IsPointInBiome(x, z)) {
                        finalColor = biome.areaColor;                           // Take biome color
                        if (!biome.isGlobalBiome) return finalColor;            // Prioritize color
                    }
                }
            }
            return finalColor;
